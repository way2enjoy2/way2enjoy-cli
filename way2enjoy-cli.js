...
if (argv.v || argv.version) {
  ...
  console.log(
    ...
    '  way2enjoy assets/img/test.png\n' +
    '  way2enjoy assets/img/test.jpg\n' +
    '\n' +
    'Options\n' +
  ...
  console.log(chalk.underline.bold('Way2enjoy CLI'));
  ...
  } else if (fs.existsSync(home + '/.way2enjoy')) {
    key = fs.readFileSync(home + '/.way2enjoy', 'utf8').trim();
  }
  ...
  if (key.length === 0) {
    ...
    console.log(chalk.bold.red('No API key specified. You can get one at ' + chalk.underline('https://way2enjoy.com/developers') + '.'));
    ...
    files.forEach(function(file) {
      if (fs.existsSync(file)) {
        if (fs.lstatSync(file).isDirectory()) {
          images = images.concat(glob.sync(file + (argv.r || argv.recursive ? '/**' : '') + '/*.+(png|jpg|jpeg|PNG|JPG|JPEG|GIF|gif)'));
        } else if (minimatch(file, '*.+(png|jpg|jpeg|PNG|JPG|JPEG|GIF|gif)', {
            matchBase: true
          })) {
          images.push(file);
        }
      ...
      unique.forEach(function(file) {
        ...
        fs.createReadStream(file).pipe(request.post('https://api.tinify.com/shrink', {
          ...
          try {
            ...
            console.log(chalk.red('\u2718 Not a valid JSON response for `' + body + '`'));
            return;
          }
